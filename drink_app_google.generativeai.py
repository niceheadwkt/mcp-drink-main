...
# --- 1. 配置 Gemini ---
# 建議將 API KEY 放入 .streamlit/secrets.toml 中：GEMINI_KEY = "你的KEY"
genai.configure(api_key=st.secrets["GEMINI_KEY"])
model = genai.GenerativeModel('gemini-2.5-flash')  # 免費層的 20 次/日確實不夠用。
#model = genai.GenerativeModel('gemini-1.5-flash') # 1.5 版的免費配額通常較多
...
async def call_mcp_and_gemini(user_input):
    ...
    try:
        ...
        raw_text = "抱歉，AI 助手目前無法處理您的請求，因為 API 配額已用盡。請稍後再試。"
        print(f"API 配額已用盡: {e}")
        # 可以選擇在程式中暫停或回傳友好的錯誤訊息給 Streamlit 介面    
    ...
    try:
        ...
        # 如果不是 JSON 或解析失敗，就當作普通對話回傳
        return raw_text

def drink_ai_agent(prompt):
    """供 Streamlit 呼叫的入口"""
    return asyncio.run(call_mcp_and_gemini(prompt))
